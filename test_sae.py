"""Invariants for TopK / ReLU+L1 SAEs. Fail if the math is wrong."""
...
def test_decoder_columns_unit_norm_after_step():
    ...
    # Break the constraint, then one optimizer step must restore it.
...
def test_relu_l1_high_sparsity():
    torch.manual_seed(4)
    W = superposition_features(8, 12, seed=4)
    sae = ReLUL1SAE(d=8, n_latents=48, l1=2.0)
...
def test_dead_latent_resample_rewrites_column():
    ...
    residual = torch.tensor([[0.0, 0.0, 3.0, 4.0]]).repeat(3, 1)  # direction (0,0,0.6,0.8)
