"""Sparse autoencoders: OpenAI TopK, ReLU+L1, and JumpReLU.

Encoder (Gao et al. / ICLR 2025):
    z = TopK(W_enc (x - b_dec) + b_enc)     # k largest kept, rest 0
Decoder:
    x_hat = W_dec z + b_dec                 # columns of W_dec are unit-norm

Papers: Bricken et al. 2023 (Towards Monosemanticity); Templeton et al. 2024
(Scaling Monosemanticity); Gao et al. ICLR 2025; Rajamanoharan et al. (JumpReLU);
...
def jump_relu(pre: torch.Tensor, threshold: torch.Tensor) -> torch.Tensor:
    """JumpReLU(z, θ) = z * H(z - θ). Threshold broadcasts over the last dim."""
...
class TopKSAE(nn.Module):
    """OpenAI-style TopK SAE with optional tied weights and AuxK dead-latent loss."""
    ...
    def __init__(
        ...
        # W_dec: (d, n) — each column is a dictionary atom.
        w = torch.randn(d, n_latents)
    ...
    def aux_loss(self, x: torch.Tensor, out: SAEOutput, dead: torch.Tensor | None) -> torch.Tensor:
        """Revive dead latents by reconstructing the residual with AuxK of them (Gao et al.)."""
        if self.aux_k <= 0:
            return x.new_zeros(())
        ...
        if dead is not None and dead.any():
            ...
            # Fall back: AuxK among latents that lost the main TopK.
            masked = pre.masked_fill(out.z != 0, -1e9)
            k_aux = min(self.aux_k, self.n_latents - self.k)
        if k_aux <= 0:
            return x.new_zeros(())
        z_aux = topk_latents(masked, k_aux)
        x_aux = z_aux @ self.W_dec.t()
...
class ReLUL1SAE(nn.Module):
    """Anthropic-style ReLU encoder with an L1 sparsity penalty on latents."""
...
class JumpReLUSAE(nn.Module):
    """JumpReLU SAE: z = pre * H(pre - θ) with a learned per-latent threshold."""

    def __init__(self, d: int, n_latents: int, threshold: float = 0.1, l0_coef: float = 0.0) -> None:
        super().__init__()
        self.d = d
        self.n_latents = n_latents
        w = F.normalize(torch.randn(d, n_latents), dim=0)
    ...
    def encode(self, x: torch.Tensor) -> torch.Tensor:
        return jump_relu(self.encode_pre(x), self.threshold)
    ...
    def forward(self, x: torch.Tensor) -> SAEOutput:
        pre = self.encode_pre(x)
        z = jump_relu(pre, self.threshold)
        return SAEOutput(x_hat=self.decode(z), z=z, pre=pre)
...
def superposition_features(d: int, n_features: int, seed: int = 0) -> torch.Tensor:
    ...
    # Take d rows × n_features cols, then re-normalize columns.
